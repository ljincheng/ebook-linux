...
GtkWidget *
do_sidebar_00(GtkWidget *do_widget)
{
  static GtkWidget *window = NULL;
  GtkWidget *sidebar,*stackSwitcher;
  GtkWidget *stack;
  GtkWidget *box,*headerTitleBox;
  GtkWidget *widget;
  GtkWidget *header;
  const gchar* pages[] = {
    "Welcome to GTK+",
  ...
  gtk_header_bar_set_custom_title(header,headerTitleBox);

      box = gtk_box_new (GTK_ORIENTATION_HORIZONTAL, 0);
      //sidebar = gtk_stack_sidebar_new ();
       sidebar=gtk_stack_switcher_new();
      //gtk_box_pack_start (GTK_BOX (box), sidebar, FALSE, FALSE, 0);
  gtk_container_add (GTK_CONTAINER (headerTitleBox), sidebar);
      //sidebar=gtk_stack_switcher_new();
...
gtk_stack_switcher_set_stack(sidebar, GTK_STACK (stack));
...
GtkWidget * book_test_window_create(void){

return do_sidebar_00(NULL);
