...
#define FOLDER_NAME "/bebebookapp/gnome-fs-directory.png"
#define FILE_NAME "/bebebookapp/gnome-fs-regular.png"
  ...
  if (!is_dir)
    {
      ...
      printf("epub file:%s\n",path);
    epubwindow =book_open_epub (path);
      if(epubwindow!=NULL)
        {
             gtk_widget_show_all(epubwindow);
          // gtk_window_set_modal(GTK_WINDOW (epubwindow), TRUE);
          //gtk_window_set_decorated(GTK_WINDOW (epubwindow), FALSE);
        }
  ...
  if (!window)
    {
      ...
      gtk_window_set_title (GTK_WINDOW (window), "book store");
      ...
      parent = g_strdup ("/workspace/ebook/mybook/");
      store = create_store ();
      fill_store (store);
...
GtkWidget *
book_get_iconview (GtkWidget *do_widget)
{
    GtkWidget *vbox;
  window=do_widget;
