...
//   "document.body.style.background = '#ffffff';"
  ...
  //               "document.body.style.background = '#121212';"
    ...
    {
           webkit_web_view_run_javascript (web_view,
   //              "document.body.style.background = '#f3eacb';"
...
void  gepub_widget_set_themes (GepubWidget *widget,   gint    themes)
{
    widget->themes = themes;
    reload_length_cb (GTK_WIDGET (widget), NULL, NULL);
}

gint     gepub_widget_get_themes    (GepubWidget *widget)
{
  return widget->themes;
}

void gepub_load_uri(GepubWidget *widget, const gchar* uri)
{
                              printf("epub:获取加载地址uri=%s\n",uri);
   gchar *path,*chapterUrl;
    g_return_if_fail (GEPUB_IS_DOC (widget->doc));
    g_return_if_fail(WEBKIT_IS_WEB_VIEW(widget));
    g_return_if_fail(uri);
    path=gepub_doc_get_file_fullpath(widget->doc,uri);
  //解析相对路径文件地址
   gchar **arr;
    gint i;
 ...
 g_free (arr);
 gint chapterId=gepub_doc_get_chapter_by_path(widget->doc,chapterUrl + 8);
   if(chapterId>=0)
	{
	gepub_doc_set_chapter(widget->doc,chapterId);
	}
  
    webkit_web_view_load_uri(WEBKIT_WEB_VIEW (widget), path);
