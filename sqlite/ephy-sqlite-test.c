...
test_create_connection (void)
{
  ...
  gchar *temporary_file = g_build_filename (g_get_tmp_dir (), "bebebook-sqlite-test.db", NULL);
  g_printf("sqlitedata file path:%s",temporary_file);
  ...
  temporary_file = g_build_filename (g_get_tmp_dir (), "directory-that-does-not-exist", "bebebook-sqlite-test.db", NULL);
...
int main(int argc,char *argv[])
{
  ...
  gchar *temporary_file = g_build_filename ("/workspace/temp/", "bebebook-sqlite-test.db", NULL);
  g_printf("sqlitedata file path:%s\n",temporary_file);
  connection = ephy_sqlite_connection_new ();


/*
  if (g_file_test (temporary_file, G_FILE_TEST_IS_REGULAR))
    {
    g_unlink (temporary_file);
      g_printf("解决文件;\n");
    }
...
if(!ephy_sqlite_connection_table_exists(connection,"bebe_books"))
    {
statement = ephy_sqlite_connection_create_statement (connection, "CREATE TABLE bebe_books (id varchar(50),title varchar(200), src LONGVARCHAR,author VARCHAR(100),sort varchar(100))", &error);
ephy_sqlite_statement_step (statement, &error);
g_object_unref (statement);
}

  statement = ephy_sqlite_connection_create_statement (connection, "INSERT INTO bebe_books (id, title) VALUES ('4', \"test5\")", &error);
  ephy_sqlite_statement_step (statement, &error);
  g_object_unref (statement);

  statement = ephy_sqlite_connection_create_statement (connection, "SELECT * FROM bebe_books", &error);
